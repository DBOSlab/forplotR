...
.clean_chr <- function(x) {
  ...
  # Normalize non-breaking spaces before any other processing
  x <- gsub(
    intToUtf8(0x00A0),
    " ",
    x,
  ...
  # Multi-character Latin substitutions
  replacements <- c(
    "\u00c6" = "AE",
    "\u00e6" = "ae",
    "\u0152" = "OE",
  ...
  # One-to-one Latin accent transliteration
  x <- chartr(
    paste0(
      "\u00c0\u00c1\u00c2\u00c3\u00c4\u00c5",
      "\u00e0\u00e1\u00e2\u00e3\u00e4\u00e5",
...
.create_rmd_content <- function(subplot_plots,
  ...
  priority_route_section <- c(
    ...
    "    pdf_table <- params$priority_species_tbl %>%",
    "      dplyr::mutate(!!names(.)[5] := gsub(",
    "        \"\\\\[([^]]+)\\\\]\\\\([^)]+\\\\)\",",
    "        \"\\\\1\",",
    "        .[[5]]",
    "      ))",
