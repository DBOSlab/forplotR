...
.create_rmd_content <- function(subplot_plots,
  ...
  # Resolve language safely even with language = language
  ...
  allowed <- c("en", "pt", "es", "ma")
  if (!language %in% allowed) {
    warning(sprintf("Invalid language '%s'. Using 'en'.", language), call. = FALSE)
    language <- "en"
  }

  df_dictionary <- tibble::tibble(
    en = c(
      "Full Plot Report", "MONITORA Program", "Back to Contents", "Contents",
      "Metadata", "Plot Name", "Plot Code", "Team", "Number of Census",
      "Dates of Census", "Specimen Counts", "Total Specimens",
      "Collected (excluding palms)", "Not Collected (excluding palms)",
      "Palms (Arecaceae)", "Dead trees since first census",
      "Recruits since first census", "Above-ground Biomass",
      "Above-ground biomass:", "AGB", "General Plot", "Collected Only",
      "Not Collected Palms", "Not Collected", "Palms", "Subplot Index",
      "Subplots", "Subplot ", "Checklist", "Individual Subplots"
    ),
    pt = c(
      "Relatório Completo da Parcela", "Programa MONITORA", "Voltar ao Sumário", "Sumário",
      "Metadados", "Nome da Parcela", "Código da Parcela", "Equipe", "Número de Censos",
      "Datas dos Censos", "Contagem de Indivíduos", "Total de Indivíduos",
      "Coletados (excluindo palmeiras)", "Não Coletados (excluindo palmeiras)",
      "Palmeiras (Arecaceae)", "Árvores mortas desde o primeiro censo",
      "Recrutas desde o primeiro censo", "Biomassa Acima do Solo",
      "Biomassa acima do solo:", "AGB", "Mapa Geral da Parcela", "Apenas Coletados",
      "Palmeiras Não Coletadas", "Não Coletados", "Palmeiras", "Índice de Subparcelas",
      "Subparcelas", "Subparcela ", "Lista de Espécies", "Subparcelas Individuais"
    ),
    es = c(
      "Informe completo de la parcela", "Programa MONITORA", "Volver al Contenido", "Contenido",
      "Metadatos", "Nombre de la Parcela", "Código de la Parcela", "Equipo", "Número de Censos",
      "Fechas de los Censos", "Conteo de Individuos", "Total de Individuos",
      "Colectados (excluyendo palmas)", "No Colectados (excluyendo palmas)",
      "Palmas (Arecaceae)", "Árboles muertos desde el primer censo",
      "Reclutas desde el primer censo", "Biomasa Aérea",
      "Biomasa aérea:", "AGB", "Mapa General de la Parcela", "Solo Colectados",
      "Palmas No Colectadas", "No Colectados", "Palmas", "Índice de Subparcelas",
      "Subparcelas", "Subparcela ", "Lista de Especies", "Subparcelas Individuales"
    ),
    ma = c(
      "样地完整报告", "MONITORA 项目", "返回目录", "目录",
      "元数据", "样地名称", "样地代码", "团队", "普查次数",
      "普查日期", "个体统计", "个体总数",
      "已采集（不含棕榈科）", "未采集（不含棕榈科）",
      "棕榈科个体 (Arecaceae)", "自第一次普查以来死亡的树木",
      "自第一次普查以来新增个体", "地上生物量",
      "地上生物量：", "地上生物量", "样地总体地图", "仅已采集个体",
      "未采集棕榈科个体", "未采集个体", "棕榈科", "子样地索引",
      "子样地", "子样地 ", "物种清单", "单个子样地"
    )
  )
  translate_lines <- function(lines, dict, lang = c("en", "pt", "es", "ma")) {
    lang <- match.arg(lang)
    if (lang == "en") return(lines)

    for (i in seq_len(nrow(dict))) {
      lines <- gsub(dict$en[[i]], dict[[lang]][[i]], lines, fixed = TRUE)
    }
    lines
  }
  ...
  # 8) Translate (if requested)
  rmd_content <- translate_lines(rmd_content, df_dictionary, language)
