...
#' non-empty voucher code is present after input harmonization, except when
#' `Collected` explicitly indicates a negative collection status.
#'
...
#'
#' @export
forplot_vouchers <- function(fp_file_path = NULL,
                             output_dir = "voucher_imgs",
  ...
  if (!"New Tag No" %in% names(fp_sheet)) {
    fp_sheet$`New Tag No` <- NA_character_
  }
  ...
  fp_sheet$`Original determination` <- as.character(
    fp_sheet$`Original determination`
  )
  fp_sheet$`New Tag No` <- as.character(fp_sheet$`New Tag No`)

  collected_norm <- .norm_txt(fp_sheet$Collected)

  has_tag <- !is.na(fp_sheet$`New Tag No`) &
    nzchar(trimws(fp_sheet$`New Tag No`))

  is_collected_yes <- collected_norm %in% c(
    "yes",
    "sim",
    "s",
    "y",
  ...
  has_voucher <- !is.na(fp_sheet$Voucher) &
    nzchar(trimws(fp_sheet$Voucher))

  rows <- has_tag & is_collected_yes
  ...
  existing_voucher_dirs <- existing_dirs[
    basename(existing_dirs) != basename(output_dir)
  ]
  ...
  for (i in seq_len(nrow(fp_sheet))) {
    ...
    genus_correct <- .clean_path_component(
      .extract_genus(fp_sheet$`Original determination`[i])
    )

    if (.is_not_collected_value(collected_raw)) {
    ...
    correct_path_norm <- normalizePath(
      correct_path,
      winslash = "/",
      mustWork = FALSE
    )

    voucher_matches <- existing_voucher_dirs[
      basename(existing_voucher_dirs) == voucher
    ]

    if (length(voucher_matches) > 0) {
      for (vm in voucher_matches) {
        vm_norm <- normalizePath(
          vm,
          winslash = "/",
          mustWork = FALSE
        )
        ...
        if (!identical(vm_norm, correct_path_norm)) {
          ...
          if (length(photos) > 0) {
            ...
            existing_dest_files <- basename(
              list.files(correct_path, full.names = FALSE)
            )
  ...
  message(
    "Rows skipped (missing voucher/family/species): ",
    n_skipped_missing_data
  )
...
.is_not_collected_value <- function(x) {
  z <- .norm_txt(x)
