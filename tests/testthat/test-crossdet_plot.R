...
testthat::test_that("crossdet_plot errors on missing input file", {
...
testthat::test_that("crossdet_plot errors when standardized inventory lacks required columns", {
  fp <- make_fake_excel()
...
testthat::test_that("crossdet_plot normalizes taxon names and omits same_determination from matches", {
  fp <- make_fake_excel()
...
testthat::test_that("crossdet_plot uses fallback scientific name normalization when genus is missing", {
  fp <- make_fake_excel()
  occ_dir <- tempfile("occdir_")
  dir.create(occ_dir, recursive = TRUE, showWarnings = FALSE)
...
testthat::test_that("crossdet_plot can use auto-download branch via mocked IPT helpers", {
  fp <- make_fake_excel()
...
testthat::test_that("crossdet_plot writes workbook when save = TRUE", {
  ...
  invisible(forplotR::crossdet_plot(
    fp_file_path = fp,
    input_type = "field_sheet",
    herbaria = "RB",
    path = occ_dir,
